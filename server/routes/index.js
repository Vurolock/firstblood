    ...
    .then(summonerData => {
      axios
        ...
        .catch(err => {
          ...
          res.json({
            message:
              'That summoner is not currently in game. Check your spelling or try again in a few seconds.'
          });
        })
        .then(spectator => {
          ...
          let promises = summoners.map(summoner => {
            ...
            return rp(options).then($ => {
              ...
              $('.Body .Row').each((i, element) => {
                ...
                // Win ratio data
                currentSummoner[champName].winrate = {
                  value: $('.WinRatio', tableRowHtml).text()
                };
                ...
                if (!$('.Text.Left', tableRowHtml).text()) {
                  ...
                  currentSummoner[champName].winrate.wins = $(
                    '.Text.Left',
                    tableRowHtml
                  )
                    .text()
                ...
                if (!$('.Text.Right', tableRowHtml).text()) {
                  ...
                  currentSummoner[champName].winrate.losses = $(
                    '.Text.Right',
                    tableRowHtml
                  )
                    .text()
                ...
                // KDA data
                currentSummoner[champName].KDA = {
                  value: $('.KDA.Cell', tableRowHtml).data('value') + ''
                };
                currentSummoner[champName].KDA.kills = $(
                  '.Kill',
                  tableRowHtml
                ).text();
                currentSummoner[champName].KDA.deaths = $(
                  '.Death',
                  tableRowHtml
                ).text();
                currentSummoner[champName].KDA.assists = $(
                  '.Assist',
                  tableRowHtml
                ).text();

                // Gold
                currentSummoner[champName].gold = $('.KDA', tableRowHtml)
                  .next()
                  .text()
                  .slice(6, -5);

                // CS
                currentSummoner[champName].cs = $('.KDA', tableRowHtml)
                  .next()
                  .next()
                  .text()
                  .slice(6, -5);
